...
同时，Facebook之类的公司也采用了“ 开放身份”的概念，但提供了更好的可用性，这就是“Facebook链接”在2008年左右比“ 开放身份”更成功的原因之一。任何人都可以使用其Facebook身份注册其他互联网服务，而现在可以使用Facebook 应用程序接口并不需要管理自己的身份。这对于可以节省身份管理成本的小型互联网初创公司以及能在密码管理方面减少时间和麻烦的用户来说非常有用。不久之后，谷歌，亚马逊，苹果和推特提供了类似的“以服务器为中心”的身份解决方案-所有这些解决方案控制了当今大多数的在线身份市场。这还包括个人浏览历史记录，社交媒体行为以及其用户的地理位置信息。

同时，越来越多一系列的举措继续致力于更多“以用户为中心”的身份解决方案的构想，例如“信任网络”计划，它起源于“优良保密协议”（PGP）运动。它们提出了非对称密码学的使用，其中任何人都可以作为身份验证者。不幸的是，这两项举措都将电子邮件地址作为标识符，这意味着它们仍依赖诸如ICANN [^ 3]之类的机构来发行这些电子邮件地址所基于的域名。由于种种原因，PGP从未得到过广泛采用。
...
许可和最小化：用户必须始终同意第三方访问其个人数据。此外，在披露个人数据时，该披露应仅涉及所需的最少数据量。
...
图片: https://uploader.shimo.im/f/15Fm1p53jmYLHz9F.png

使用“去中心化标识符（DIDs）”的以用户为中心的身份

区块链网络目前仅提供最少的一组身份属性，这些属性不足以通过网页进行许多社会经济互动。但是，如果设置正确，则分类账可以为以用户为中心和隐私保护的身份管理系统提供关键组件，从而为所有参与人员提供更少的摩擦并降低成本。去中心化标识符（DID）与分布式分类账相结合，可实现更复杂的身份管理系统。
...
分布式分类账固有的公钥基础结构允许以公开可验证的方式注册所有参与者的DID。激活新的区块链钱包时，任何用户都可以创建和注册DID，从而创建一对私钥和公钥。任何DID都可以链接到其他人和机构发行的凭证，这些凭证证明身份所有者的特定特征（他们对自己身份的声明），例如姓名，地址，电子邮件，年龄，现有文凭或其他证明，例如驾照。
...
身份验证者: 是在验证某些与身份相关的属性后提供服务的第三方。例如，如果购买酒类或看电影有年龄限制，则身份验证者（商店或电影院）可以验证这个发行并证明该凭证的政府签名。

图片: https://uploader.shimo.im/f/tn9Moepb3mGizVtQ.png
...
身份发行者和身份所有者都需要使用其DID在公共分类账中注册。在这种设置中，分布式分类账可用于证实数据和证书的真实性，而出于验证目的，仅注册间接“指针”。分布式分类账可以用作公共基础设施，以方便验证与身份相关的数据。区块链网络中的任何人现在都可以验证身份所有者做出的声明是否有效以及哪些机构证明了该声明的有效性，而无需透露数据本身。

将私钥与DID配对可以使身份所有者创建一个QR码，例如，表示那个已验证标识符的QR码。扫描QR码，服务提供商现在可以通过区块链网络或类似的分布式分类账运行数据，并验证此证书是否与该人的DID相关联。公用密钥用于证明与凭证关联的发行机构签名的真实性。如果证书和DID匹配，则授予访问权限，并且此人有资格购买酒，租车等。除此类证书外，任何其他数据也可以与DID关联，并由身份所有者通过钱包软件直接进行控制。此类“未经验证”的数据的示例是个人浏览历史或社交媒体帖子。在这样的设置中，不再依赖第三方数字身份提供商，例如谷歌或Facebook。“Brave浏览器”是一个有关Web3钱包如何直接控制你的数字足迹的实用示例（更多内容：第4部分-基本注意代币）。
...
用户可以在激活区块链钱包时创建并注册一个DID，这将创建一对私钥和公钥。公钥加密用于身份验证和加密。只有私钥才能证明自己的身份。私钥充当你用在钱包上的个人锁。
